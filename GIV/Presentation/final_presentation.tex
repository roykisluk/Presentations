        ...
        \begin{column}{0.55\textwidth}
            ...
            \begin{enumerate}
                \item {In a standard economy, micro-shocks cancel out at a rate of $1/\sqrt{N}$ \hyperlink{app_sqrtn}{\beamergotobutton{Proof}}
                      \vspace{0.4cm}}
                \item {In a \textbf{granular} economy, size distribution of firms follows a power law: $size=\frac{1}{rank^{\frac{1}{\alpha}}}$
                      \begin{itemize}
                          \item \textbf{Zipf's Law}: $\alpha=1 \implies \text{size} \propto \frac{1}{\text{rank}}$. First firm is x2 the second, x3 the third, and so on.
                          \item \textbf{Israel}: $\alpha=1.2$. First firm is x1.8 the second, and x2.6 the third.
                          \item Large firms' idiosyncratic shocks drive aggregate outcomes
                      \end{itemize}}
            \end{enumerate}
        \end{column}
...
\begin{frame} \label{src_assumptions}
    ...
    \begin{align}
        w_t    & = \psi L_{St} + \varepsilon_t \tag{Aggregate Supply}      \\[0.5cm]
        L_{it} & = \phi^d w_t + \eta_t + u_{it} \tag{Firm-specific Demand}
    \end{align}

    Where:
    \begin{itemize}
        \item $w_t$ - wage (observed)
        \item $L_{it}$ - number of workers at firm $i$ (observed) \hyperlink{app_aggregate_demand}{\beamergotobutton{Aggregation}}
...
\begin{frame} \label{src_simple_giv_fails}
    ...
    \vspace{0.5cm}
    If size is correlated with the macro shock, the simple GIV becomes: \hyperlink{app_simple_giv_fails}{\beamergotobutton{Proof}}
    \begin{equation*}
...
\begin{frame}
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item Largest firms account for disproportionate share of employment: \\ top 10 = 15\%, top 50 = 30\%
        \item Size distribution of tech in Israel exhibits strong power law:
              \begin{equation*}
                  \frac{Q(\bar{q_i})}{Q(\bar{q_1})} = \frac{1}{i^{\frac{1}{\zeta}}}, \quad \zeta \approx 1.2
              \end{equation*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% From Firm to Aggregate Demand
\begin{frame} \label{app_aggregate_demand}
    ...
    \begin{align*}
        E_t & = \sum_{i=1}^N E_{it} = \sum_{i=1}^N \bar{E} S_i (1 + L_{it}) \\
            & = \bar{E} \sum_{i=1}^N S_i + \bar{E} \sum_{i=1}^N S_i L_{it}
    \end{align*}
    ...
    {
    \large
    \textbf{Conclusion:} The size-weighted average $L_{St}$ captures the aggregate variation in labor demand.
    }
...
\begin{frame}{Assumptions}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            ...
            \begin{description}
                \item[A1] Factor loadings $\lambda_i$ are known (relaxed in §IV.B, estimated using GMM / PCA).
...
\begin{frame}{Econometrics Refresher} \label{app_refresher}
    \begin{block}{OLS and the Annihilator Matrix}
        ...
        Slide $\Gamma \equiv$ Standard $e$ (Residuals/Granular Weights) \hyperlink{src_propositions}{\beamerreturnbutton{Back}}
    \end{block}
\end{frame}
