...
\begin{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            ...
            \vspace{0.8cm}
            \begin{enumerate}
                \item {In a standard economy, micro-shocks cancel out at a rate of $1/\sqrt{N}$ \hyperlink{app:sqrtn}{\beamergotobutton{Proof}}
                      \vspace{0.4cm}}
                \item {In a \textbf{granular} economy, where the size distribution of firms follows a power law, a few firms are so large that their idiosyncratic luck drives aggregate volatility.}
            \end{enumerate}
        \end{column}
...
\begin{frame}[shrink]
    ...
    Consider an economy with $N$ firms.
    ...
    Where:
    \begin{itemize}
        \item $w_t$ is the wage
        \item $L_{St} = \sum_i S_i L_{it}$ is the size-weighted average number of workers
        \item $\eta_t$ and $\varepsilon_t$ are aggregate shocks
        \item $\psi$ is the labor supply elasticity, $\phi^d$ is the labor demand elasticity
        \item $u_{it}$ is the idiosyncratic shock to firm $i$
    \end{itemize}
    \vspace{0.5cm}
    We cannot estimate $\psi$ and $\phi^d$ by OLS (simultaneity).
\end{frame}
...
\begin{frame}
    ...
    \vspace{0.5cm}
    Substitute the demand equation into the aggregate definitions: \hyperlink{app:equal_weighted}{\beamergotobutton{Proof}}
    \begin{align}
    ...
    \begin{equation}
        z_t = u_{\Gamma t} = u_{St} - u_{Et} =\underbrace{\sum S_i L_{it}}_{\text{Size-Weighted}} - \underbrace{\frac{1}{N} \sum L_{it}}_{\text{Equal-Weighted}} \tag{GIV}
    \end{equation}
...
\begin{frame}
    ...
    \vspace{0.5cm}
    Using the supply equation, $\mathbb{E}[(w_t - \psi L_{St}) z_t] = 0$: \hyperlink{app:psi_identification}{\beamergotobutton{Proof}}
    \begin{equation*}
...
\begin{frame}
    ...
    \vspace{0.5cm}
    The simple GIV becomes: \hyperlink{app:simple_giv_fails}{\beamergotobutton{Proof}}
    \begin{equation*}
...
\begin{frame}
    \frametitle{Why not just Firm Size ($S_i$)?}
    \begin{itemize}
        ...
        \item \textbf{The Conflict:} If we use $z_t = \sum (S_i - 1/N) L_{it}$, we have power, but we lack validity if $S_i$ is correlated with $\lambda_i$ (e.g., if big firms are more sensitive to macro shocks).
        ...
        \item \textbf{The math:} In matrix form, we want $\Gamma$ to be as close as possible to $S$ but satisfy $\Gamma \perp \lambda$. This is a standard projection problem: $\Gamma = S - \lambda (\lambda' \lambda)^{-1} \lambda' S$
        \item \textbf{Intuition:} We take the firm sizes and subtract the portion of size that is explained by macro-sensitivity. What remains is "Pure Granular Weight."
    \end{itemize}
...
\begin{frame}
    ...
    \begin{align*}
        z_t & = \sum \Gamma_i (\phi w_t + \lambda_i \eta_t + u_{it})                               \\
    ...
    For this, we use the $N \times N$ projection matrix $Q$ that is orthogonal to the factor loadings—that is, $Q \lambda = 0$:
    \begin{equation*}
        Q = I - \lambda (\lambda' \lambda)^{-1} \lambda'
    \end{equation*}
    The optimal weights are then given by $\Gamma^{*\prime} = S'Q$. \hyperlink{app:propositions}{\beamergotobutton{Propositions}}
...
%     \begin{itemize}
%         \item \textbf{Principal Component Analysis (PCA):} We apply PCA to the panel data of firm outcomes ($L_{it}$) to extract the largest sources of common variance.
...
\begin{frame} \label{app:size_weighted}
...
\begin{frame} \label{app:equal_weighted}
...
\begin{frame} \label{app:psi_identification}
...
\begin{frame} \label{app:simple_giv_fails}
...
\begin{frame}{Granular Instrumental Variables isolate idiosyncratic shocks} \label{app:propositions}
    \vspace{0.2cm}
    \centering
