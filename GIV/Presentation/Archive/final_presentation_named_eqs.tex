...
\begin{frame}
    ...
    \vspace{0.5cm}
    \begin{align}
        w_t    & = \psi L_{St} + \varepsilon_t \tag{Supply} \label{eq:supply} \\[0.5cm]
        L_{it} & = \phi^d w_t + \eta_t + u_{it} \tag{Firm Demand} \label{eq:firm_demand}
    \end{align}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item This implies that $L_{St}$ is correlated with $\varepsilon_t$ in the supply equation \eqref{eq:supply}.
        \item And $w_t$ is correlated with $\eta_t$ in the demand equation \eqref{eq:firm_demand}.
    \end{itemize}
...
\begin{frame}
    ...
    \begin{equation}
        L_{Et} = \frac{1}{N} \sum_{i=1}^N L_{it} \tag{Equal-Weighted Average} \label{eq:equal_weight_avg}
    \end{equation}

    \vspace{0.5cm}
    The GIV is the size-weighted outcome minus the equal-weighted outcome:
    \begin{equation}
        z_t := L_{\Gamma t} = L_{St} - L_{Et} \tag{GIV Definition} \label{eq:giv_def}
    \end{equation}
...
\begin{frame}
    ...
    \begin{align}
        ...
        L_{Et} & = \phi^d w_t + \eta_t + u_{Et} \tag{Aggregate Demands} \label{eq:agg_demands}
    \end{align}

    \vspace{0.5cm}
    The common components ($\phi^d w_t + \eta_t$) difference out:
    \begin{equation}
        z_t = u_{\Gamma t} = u_{St} - u_{Et} =\underbrace{\sum S_i L_{it}}_{\text{Size-Weighted}} - \underbrace{\frac{1}{N} \sum L_{it}}_{\text{Equal-Weighted}} \tag{Isolated Shocks} \label{eq:isolated_shocks}
    \end{equation}
...
\begin{frame}
    ...
    \vspace{0.5cm}
    Using the supply equation \eqref{eq:supply}, $\mathbb{E}[(w_t - \psi L_{St}) z_t] = 0$:
    \begin{equation}
        \psi = \frac{\mathbb{E}[w_t z_t]}{\mathbb{E}[L_{St} z_t]} \tag{Supply Elasticity} \label{eq:psi_est}
    \end{equation}

    \vspace{0.5cm}
    Using the equal-weighted demand equation from \eqref{eq:agg_demands}, $\mathbb{E}[(L_{Et} - \phi^d w_t) z_t] = 0$:
    \begin{equation}
        \phi^d = \frac{\mathbb{E}[L_{Et} z_t]}{\mathbb{E}[w_t z_t]} \tag{Demand Elasticity} \label{eq:phi_est}
    \end{equation}
...
\begin{frame}
    ...
    \vspace{0.5cm}
    \begin{equation}
        L_{it} = \phi w_t + \lambda_i \eta_t + u_{it} \tag{Heterogeneous Loadings} \label{eq:het_loadings}
    \end{equation}
    \vspace{0.5cm}
    The simple GIV becomes:
    \begin{equation}
        z_t = (\lambda_S - \lambda_E) \eta_t + (u_{St} - u_{Et}) \tag{Leaky GIV} \label{eq:leaky_giv}
    \end{equation}
    \vspace{0.5cm}
    {\Large
...
\begin{frame}
    ...
    \vspace{1cm}
    \begin{equation}
        \sum_{i=1}^N \Gamma_i \lambda_i = 0 \quad \text{(Orthogonality Condition)} \tag{Orthogonality} \label{eq:ortho}
    \end{equation}
    \vspace{0.5cm}
    {\Large
...
\begin{frame}
    ...
    \begin{equation}
        ...
        \frac{\sigma^2}{N} \tag{Variance Decay} \label{eq:var_decay}
    \end{equation}
